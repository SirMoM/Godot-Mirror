...
func put(level, message, module = default_module):
...
func get_module(module = default_module):
...
static func format(template, level, module, message):
	...
	var second = "%02d" % time_dict["second"]
	...
	output = output.replace(FORMAT_IDS.time, time)
