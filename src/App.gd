...
onready var progress_bar := $MainUI/ProgressBar 
onready var mirror_button := $MainUI/MirrorButton
onready var selector: OptionButton = $MainUI/HBoxContainer/ConfigFileSelector
...
func _on_MirrorButton_pressed():
	...
	for input in mirror_config["input"]:
		...
		progress_bar.value += 100 / mirror_config["input"].size()
